<template>
  <div class="recruit">
    ...
    <div class="recruitContent">
      ...
      <div v-if="recruitShow">
        <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="recruitShow" width="50%"
                   height="90%">
...
export default {
  ...
  data: function () {
    return {
      ...
      businessId: '',
      updateData: {
        metadata: '{"system":"vol","module":"recruit","businessId":' + this.businessId + '}'
      },
  ...
  methods: {
    ...
    onSuccess(res) {
      ...
      this.$axios.post(window.volConfig.volUrl + 'recruit/content/upload', postData).then(resp => {
    ...
    deleteSingle(data) {
      this.$confirm('是否刪除?', '刪除', {
        ...
        this.$axios.post(window.volConfig.volUrl + 'recruit/content/delete', data).then(res => {
          if (res.data.success) {
            this.search();
            this.$message({
              type: 'success',
    ...
    // 发布
    release(data) {
      this.$axios.post(window.volConfig.volUrl + 'recruit/content/release', data).then(res => {
        const data = res.data;
        if (data.success) {
    ...
    // 获取奖励内容管理列表
    getRecruitContentList() {
      this.$axios.get(window.volConfig.volUrl + 'recruit/content/list/' + this.currentPage + '/' + this.pageSize).then(res => {
