  ...
  <div class="reward">
    ...
    <div class="rewardContent">
      <el-table
        ...
        <el-table-column prop="updatorName" sortable label="上傳人" width="300px" :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="updateTime" sortable label="上傳時間" width="350px" :formatter="formatDate"
                         :show-overflow-tooltip="true"></el-table-column>
        <el-table-column prop="releaseTime" sortable label="發佈時間" width="350px" :formatter="formatDate"
                         :show-overflow-tooltip="true"></el-table-column>
      ...
      <div v-if="rewardShow">
        <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" :visible.sync="rewardShow" width="50%"
                   height="90%">
...
export default {
  ...
  data: function () {
    return {
      ...
      businessId: '',
      updateData: {
        metadata: '{"system":"vol","module":"reward","businessId":' + this.businessId + '}'
      },
  ...
  methods: {
    ...
    onSuccess(res) {
      ...
      this.$axios.post(window.volConfig.volUrl + 'reward/content/upload', postData).then(resp => {
    ...
    deleteSingle(data) {
      this.$confirm('是否刪除?', '刪除', {
        ...
        this.$axios.post(window.volConfig.volUrl + 'reward/content/delete', data).then(res => {
          if (res.data.success) {
            this.search();
            this.$message({
              type: 'success',
    ...
    // 发布
    release(data) {
      this.$axios.post(window.volConfig.volUrl + 'reward/content/release', data).then(res => {
        const data = res.data;
        if (data.success) {
    ...
    getRewardContentList() {
      this.$axios.get(window.volConfig.volUrl + 'reward/content/list/' + this.currentPage + '/' + this.pageSize).then(res => {
