...
@Component({
  selector: 'app-buffprotection-target',
  templateUrl: './buffprotection-target.component.html'
  // styleUrls: ['./buffprotection-target.component.css']
})
export class BuffProtectionTargetComponent implements OnInit {
  ...
  constructor(private surroundAnalysisService: PerimeterService,    // 事件类型服务
    ...
    this.resourceTypeCriteria.parentCode = 'A05B02';
  ...
  siteIncident() {
    ...
    this.point = this.locationService.influencePoint('targetLo', 'targetLa');
    return this.point;
  }
  ...
  showDetail(i, shelter) {
    ...
    // 定位、详情
    this.tsemapService.protectionObjDetail(shelter);
  }
  ...
  query(querymodel) {
    ...
    this.locationService.locationQuery(this.bufferType, $('.targetLo').val(), $('.targetLa').val(), this.radus * 1000, 'A05B02','defenceobj');
  }
