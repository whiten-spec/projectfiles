...
export class StoreAreaComponent implements OnInit {
    ...
    getData() {
        this.httpServer.post('/rms/store/setting/zone/list', {}).then((res: any) => {
           // console.log(res);
            if (res.success) {
    ...
    save() {
     //   console.log(this.addModel);
        ...
        if (this.disabledFlag === '1') {
            this.httpServer.post('/rms/store/setting/zone/add', this.addModel).then((res: any) => {
             //   console.log(res);
                if (res.success) {
                    this.getData();
            ...
            this.httpServer.post('/rms/store/setting/zone/update', this.addModel).then((res: any) => {
            //    console.log(res);
                if (res.success) {
                    this.getData();
                    this.addVisiable= false;
    ...
    // 刪除操作
    confirm_click() {
        this.httpServer.post('/rms/store/setting/zone/deleteById', this.curData)
        .then((result) => {
